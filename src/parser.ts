...
export const parse = async (inputCSVFile: string): Promise<TParsingResult> => {
  try {
    ...
    for (const items of recordItemMap.values()) {
      const recordDebit = round2(
        items.reduce((sum, item) => sum + item.debit, 0)
      );
      const recordCredit = round2(
        items.reduce((sum, item) => sum + item.credit, 0)
      );

      if (recordDebit === recordCredit) {
        groupedRecords.push({
          recordItems: items,
          totalDebit: recordDebit,
          totalCredit: recordCredit,
        });
      }
    }
